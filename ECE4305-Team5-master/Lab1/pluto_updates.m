...
cdBuiltin = comm.ConstellationDiagram('ReferenceConstellation', [-1 1],...
    'SymbolsToDisplaySource','Property',...
    'SymbolsToDisplay',frameSize/2,...
    'Name','PLL Baseband with Timing Offset');
...
%% Setting up Pluto
% Setup Receiver
rx=sdrrx('Pluto','OutputDataType','double','SamplesPerFrame',frameSize*SPS);
% Setup Transmitter
tx = sdrtx('Pluto','Gain',-29);

evm = comm.EVM('MaximumEVMOutputPort',true);

tx.transmitRepeat(filteredTXData);
% Setup Scope
...
'BufferLength', rx.SamplesPerFrame*steps,'ShowLegend', true);
cd = comm.ConstellationDiagram('ReferenceConstellation', [-1 1],...
    'Name','Baseband');
...
rxFilPluto = step(RxFlt, rxPluto);
stop=1;
step(cd,rxFilPluto); pause(0.01);

%% EVM Measurement
close all
[rmsEVM_1,maxEVM_1] = evm(RxFlt,rxPluto)
