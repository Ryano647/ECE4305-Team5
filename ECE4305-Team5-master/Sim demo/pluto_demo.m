...
for k=1:frameSize:(numSamples)
    ...
    release(tx);
    tx.transmitRepeat(RxFlt(allfilteredTXData));
    plutoData = RxFlt(rx());
    ...
    offsetData(i) = plutoData(i) *offset(i);
    end
    if k==1
...
%         end
        ...
        intOut = step(filt,filteredData(i+5),fliplr(b(Mu).'));
        if i > 2
...
state = 0;
downsampledData = [];
for i=1:2:length(allintOut)
    downsampledData = [downsampledData, allintOut(i)];
end

downsampledData = downsampledData';

demodulatedData = demod.step(downsampledData);
gb =0;
for i=1:length(data)
    if demodulatedData(i) == data(i)
        gb=gb+1;
    end
end
...
% % 
% % % Pass data through radio
% % tx.transmitRepeat(RxFlt(modulatedData));
% % plutoData = RxFlt(rx());
% % % Set up visualization and delay objects
% VFD = dsp.VariableFractionalDelay; cd = comm.ConstellationDiagram;
% % Process received data for timing offset
...
% demodulatedData = demodulatedData(end-remainingSPS*1000+1:end);
% for index = 0:300
%     % Delay signal
%     tau_hat = index/50;
%     delayedsig = VFD(demodulatedData, tau_hat);
